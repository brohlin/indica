...
public class Carpeta extends HttpServlet
{
	...
	protected ByteArrayOutputStream generatePDFDocumentBytes(
		...
		{	
			...
			int totalPagesQuitarRefugiadoReconocido = 0;	
			...
			boolean QuitarRefugiadoReconocido = false;
			...
			PdfReader pdfReaderQuitarRefugiadoReconocido = null;
			
			java.sql.Blob blobPasaporte = null;
			java.sql.Blob blobOtroDocumento = null;
			java.sql.Blob blobDJ = null;
			java.sql.Blob blobES = null;
			java.sql.Blob blobFoto = null;
			java.sql.Blob blobEL = null;
			java.sql.Blob blobAdmRecon = null;
			java.sql.Blob blobAdmApel = null;
			java.sql.Blob blobRefRecon = null;
			java.sql.Blob blobRefApel = null;
			java.sql.Blob blobRenuncia = null;
			java.sql.Blob blobQuitarRefugiadoReconocido = null;
			...
			try{
				if (session.getAttribute("email") != null) {
					...
					{
						size++;
						blobPasaporte = rs.getBlob(1);
						blobOtroDocumento = rs.getBlob(2);
						blobDJ = rs.getBlob(3);
						blobES = rs.getBlob(4);
						blobFoto = rs.getBlob(5);
						blobEL = rs.getBlob(6);
						blobAdmRecon = rs.getBlob(7);
						blobAdmApel = rs.getBlob(8);
						blobRefRecon = rs.getBlob(9);
						blobRefApel = rs.getBlob(10);
						blobRenuncia = rs.getBlob(11);
						blobQuitarRefugiadoReconocido = rs.getBlob(12);	
					}
			...
			if (session.getAttribute("temp_solicitante_v2_quitar_refug_recon") != null) {
				try {
					...
					InputStream isQuitarRefugiadoReconocido = blobQuitarRefugiadoReconocido.getBinaryStream();
					pdfReaderQuitarRefugiadoReconocido = new PdfReader(isQuitarRefugiadoReconocido);
					totalPagesQuitarRefugiadoReconocido  = pdfReaderQuitarRefugiadoReconocido.getNumberOfPages();
					QuitarRefugiadoReconocido = true;
				} catch(Exception e) {
					// do nothing
				}
			}
			try
			{
	            ...
	            // Quitar Refugiado Reconocido
                if(QuitarRefugiadoReconocido) {
	                while (pageOfCurrentReaderPDF < totalPagesQuitarRefugiadoReconocido) {  
	                    doc.newPage();  
	                    pageOfCurrentReaderPDF++;  
	                    currentPageNumber++;  
	                    page = docWriter.getImportedPage(pdfReaderQuitarRefugiadoReconocido, pageOfCurrentReaderPDF);  
	                    cb.addTemplate(page, 0, 0);  
	                    ...
	                    if (paginate) {  
	                        ...
	                        cb.setFontAndSize(bf, 9);  
	                        cb.showTextAligned(PdfContentByte.ALIGN_CENTER, ""  
	                                + currentPageNumber + " of " + totalPagesQuitarRefugiadoReconocido, 520,  
	                                5, 0);  
	                        cb.endText();  
	                    }  
	                }  
